...
`);
        ...
        const range = import_coc2.Range.create(doc.textDocument.positionAt(startPosition), doc.textDocument.positionAt(endPosition));
        const options = {
  ...
  const runOnProject = import_coc2.commands.registerCommand("tailwindCSS.headwind.sortTailwindClassesOnWorkspace", () => {
    const workspaceFolder = import_coc2.workspace.workspaceFolders || [];
    if (workspaceFolder[0]) {
      const workspacePath = import_coc2.Uri.parse(workspaceFolder[0].uri).fsPath;
      import_coc2.window.showInformationMessage(`Running Headwind on: ${workspacePath}`);
      ...
      rustyWindProc.stderr.on("data", (data) => {
        if (data && data.toString() !== "") {
          ...
          import_coc2.window.showErrorMessage(`Headwind error: ${data.toString()}`);
        }
      });
    }
  });
  ...
  if (config.get("tailwindCSS.headwind.runOnSave")) {
    context.subscriptions.push(import_coc2.workspace.onWillSaveTextDocument((_e) => {
...
function isExcluded(file, folder) {
  ...
  for (const pattern of exclude) {
    if ((0, import_minimatch.default)(file, import_path2.default.join(import_coc3.Uri.parse(folder.uri).fsPath, pattern))) {
      return true;
    }
  }
  return false;
...
async function activate2(context) {
  if (!getConfigTailwindCssEnable())
    return;
  context.subscriptions.push(import_coc3.commands.registerCommand("tailwindCSS.forceActivate", async () => {
    import_coc3.window.showInformationMessage(`Force activation of coc-tailwindcss3`);
  }));
  let module2 = getConfigCustomServerPath();
  if (module2 && import_fs.default.existsSync(module2)) {
    module2 = module2;
  } else {
    module2 = context.asAbsolutePath(import_path2.default.join("node_modules", "@tailwindcss", "language-server", "bin", "tailwindcss-language-server"));
  }
  const outputChannel = import_coc3.window.createOutputChannel("tailwindcss-language-server");
  context.subscriptions.push(import_coc3.commands.registerCommand("tailwindCSS.showOutput", () => {
    if (outputChannel) {
      outputChannel.show();
    }
  }));
  const watcher = import_coc3.workspace.createFileSystemWatcher(`**/${CONFIG_FILE_GLOB}`, false, true, true);
  ...
  context.subscriptions.push(import_coc3.workspace.onDidChangeConfiguration((event) => {
  ...
  function bootWorkspaceClient(folder) {
    ...
    const clientOptions = {
      ...
      outputChannel,
      synchronize: {
        fileEvents: import_coc3.workspace.createFileSystemWatcher(CONFIG_FILE_GLOB)
      },
      middleware: {
        workspace: {
  ...
  async function didOpenTextDocument(document) {
    ...
    folder = getOuterMostWorkspaceFolder(folder);
    try {
      const configFiles = await (0, import_fast_glob.default)(import_path2.default.join(import_coc3.Uri.parse(folder.uri).fsPath, "**/" + CONFIG_FILE_GLOB), {
  ...
  context.subscriptions.push(import_coc3.workspace.onDidChangeWorkspaceFolders((event) => {
